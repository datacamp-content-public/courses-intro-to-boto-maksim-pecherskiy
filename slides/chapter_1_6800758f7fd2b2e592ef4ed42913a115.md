...
Now that you have these buckets, it's time to put stuff in them. The files within S3 buckets are called objects.  

An object can be anything - an image, a video file, CSV or a log file.  

Uploading, downloading, and working with object metadata is a key component of many data pipelines.  
...
`@part1`
- Buckets contain many objects {{1}}
...
First we create our resource by calling the the boto3 resource method with the argument 's3'.

--

Then we create a new instance of an object, passing bucket name and key as arguments. 

--

This does not mean there is an object in the bucket yet. It's just a reference, and right now it points to an empty space.

Let's fill that empty space with the contents of our trout_obs.csv. We call the object instance's upload_file method, and pass the local file name as an argument.  
...
--
If we want to update a file, we use its key to overwrite it. That means we have to be really careful to make sure we are not overwriting things we shouldn't when we upload.
...
Create a new instance of an object. This time, the object already exists.
