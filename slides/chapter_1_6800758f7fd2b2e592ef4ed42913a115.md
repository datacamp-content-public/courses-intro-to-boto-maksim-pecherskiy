...
`@part1`
- Bucket contains many objects {{1}}
- Object has only one parent bucket {{2}}
- Bucket can list its children objects {{3}}
- Object can access its parent bucket {{4}}
- Buckets have unique names {{5}}
- Objects have unique keys {{6}}
...
Every object must have at least one parent bucket, that it contains a reference to.  

Buckets can contain many objects.

A bucket can access its children objects.

An object can access its parent bucket.
